...
describe("Comprehensifier", () => {
	it("should encode to binary base", () => {
		...
		expect(comprehensifier.comprehensify(Uint8Array.from([1, 2, 3, 4, 5, 6, 7]))).to.equal((283686952306183).toString(2));
	});
	...
	it("should decode from binary base", () => {
		const comprehensifier = new BinaryComprehensifier();
		expect(comprehensifier.uncomprehensify((0).toString(2))).to.deep.equal(Uint8Array.of(0));
		expect(comprehensifier.uncomprehensify((222).toString(2))).to.deep.equal(Uint8Array.of(222));
	});

	it("should encode to large base", () => {
		const comprehensifier = new LargeBaseComprehensifier();
		expect(comprehensifier.uncomprehensify("0")).to.deep.equal(Uint8Array.of(0));
		expect(comprehensifier.uncomprehensify("40")).to.deep.equal(Uint8Array.of(40));
		expect(comprehensifier.uncomprehensify("1:0")).to.deep.equal(Uint8Array.of(41));
	});
